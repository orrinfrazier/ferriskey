...
pub async fn run_cluster_controller(client: Client, service: Arc<OperatorService>) {
    ...
    Controller::new(clusters, Config::default())
        ...
        .for_each(|res| async move {
            match res {
                Ok((obj, _)) => tracing::info!("reconciled cluster: {:?}", obj.name),
                Err(e) => tracing::warn!("reconciled failed cluster: {:?}", e),
            }
        })
        .await;
}
...
async fn reconcile(
    ...
    let action = finalizer(&api, FINALIZER, cluster, |event| async {
        match event {
            Event::Apply(obj) => {
                match service.reconcile_cluster(&spec, &ns).await {
                    ...
                    Err(e) => {
                        ...
                        .await
                        {
                            tracing::warn!("failed to update status: {:?}", status_err);
                        }
            ...
            Event::Cleanup(_) => {
                tracing::info!("starting cleanup for cluster: {}", spec.name);

                service.cleanup_cluster(&spec, &ns).await?;

                tracing::info!("cleanup completed for cluster: {}", spec.name);
    ...
    match action {
        ...
        Err(e) => match &e {
            kube::runtime::finalizer::Error::RemoveFinalizer(kube::Error::Api(api_err))
                if api_err.code == 404 =>
            {
                tracing::info!("resource already deleted, finalizer removal completed");

                Ok(Action::await_change())
            }
            _ => {
                tracing::error!("error finalizer: {:?}", e);
...
async fn update_status(
    ...
    tracing::info!("status updated for cluster: {}", name);

    Ok(())
}

fn error_policy(cluster: Arc<FerrisKeyCluster>, err: &OperatorError, _: Arc<()>) -> Action {
    tracing::warn!("error reconciling {:?}: {:?}", cluster.name_any(), err);
